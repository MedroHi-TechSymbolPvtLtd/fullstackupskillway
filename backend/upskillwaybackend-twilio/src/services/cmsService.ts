...
import { applyProgramFilters, ProgramFilterOptions } from '../utils/programFilter';
import { ProgramFilterQuery } from '../utils/validation';
import {
  BlogInput,
  UpdateBlogInput,
  VideoInput,
...
// Fixed resolveCreatedBy function to handle foreign key constraints properly
const resolveCreatedBy = async (actorId: string): Promise<string> => {
  // If actorId is provided and not 'admin', validate it exists
  ...
  if (!adminUser) {
    ...
    adminUser = await prisma.user.create({
      data: {
        ...
        passwordHash: 'default-hash', // You should hash this properly
        role: 'admin'
      }
    });
    logger.info('Created default admin user:', adminUser.id);
  }
...
export const getAllCourses = async (
  ...
  limit: number = 10,
  status?: string,
  filters?: ProgramFilterQuery
) => {
  try {
    const skip = (page - 1) * limit;
    const where: Prisma.CourseWhereInput = status ? { status: status as any } : {};

    const filterOptions: ProgramFilterOptions | undefined = filters
      ? {
          ...filters,
          fields: {
            price: 'price',
            duration: ['durationMonths', 'durationHours'],
            keyword: ['title', 'description'],
          },
        }
      : undefined;

    applyProgramFilters(where, filterOptions);
...
export const getCourseCurriculum = async (courseId: string) => {
  try {
    ...
    logger.error(`Get course curriculum failed: ${courseId}`, error);
    throw error;
  }
};

export const updateCourseCurriculum = async (courseId: string, curriculumData: CourseCurriculumInput) => {
  try {
    const course = await prisma.course.update({
      where: { id: courseId },
      data: {
    ...
    logger.info(`Course curriculum updated: ${course.title}`);
    return {
      id: course.id,
      title: course.title,
      curriculum: course.curriculum ?? [],
    ...
    logger.error(`Update course curriculum failed: ${courseId}`, error);
...
export const patchCourseShortDescription = async (
  courseId: string,
  payload: CourseShortDescriptionInput
) => {
  try {
    const course = await prisma.course.update({
      where: { id: courseId },
    ...
    logger.info(`Course short description updated: ${course.title}`);
    return course;
  } catch (error) {
    logger.error(`Patch course short description failed: ${courseId}`, error);
    if ((error as any)?.code === 'P2025') {
      throw new NotFoundError('Course not found');
    }
    throw error;
...
export const getCourseMentors = async (courseId: string) => {
  try {
    const course = await prisma.course.findUnique({
      where: { id: courseId },
      select: {
    ...
    logger.error(`Get course mentors failed: ${courseId}`, error);
    throw error;
  }
};

export const updateCourseMentors = async (courseId: string, payload: CourseMentorInput) => {
  try {
    const course = await prisma.course.update({
      where: { id: courseId },
      data: {
        mentors: payload.mentors,
        mentorSocialLinks: payload.mentorSocialLinks,
      },
      select: {
        id: true,
        title: true,
    ...
    logger.info(`Course mentors updated: ${course.title}`);
    return {
      id: course.id,
      title: course.title,
      mentors: course.mentors ?? [],
    ...
    logger.error(`Update course mentors failed: ${courseId}`, error);
    if ((error as any)?.code === 'P2025') {
      throw new NotFoundError('Course not found');
    }
    throw error;
...
export const getAllStudyAbroad = async (
  page: number = 1,
  limit: number = 10,
  status?: string,
  filters?: ProgramFilterQuery
  ...
  try {
    const skip = (page - 1) * limit;
    const where: Prisma.StudyAbroadWhereInput = status ? { status: status as any } : {};
    ...
    const filterOptions: ProgramFilterOptions | undefined = filters
      ? {
          ...
          fields: {
            ...
            partTime: 'partTimeAvailable',
            university: 'universities',
            city: 'city',
            keyword: ['city', 'description'],
          },
        }
      : undefined;
...
export const getAllTrainingPrograms = async (
  ...
  try {
    ...
    // Filter by programTrainingType (uppercase enum)
    if (programTrainingType) {
      where.programTrainingType = programTrainingType.toUpperCase() as any;
    }
    ...
    logger.error('Get all training programs failed', error);
    throw error;
  }
};

export const getTrainingProgramById = async (trainingProgramId: string) => {
  try {
    ...
    logger.error(`Get training program failed: ${trainingProgramId}`, error);
    throw error;
  }
};

export const createTrainingProgram = async (trainingProgramData: any, createdBy: string) => {
  try {
    const existingProgram = await prisma.trainingProgram.findUnique({
      where: { slug: trainingProgramData.slug },
    });
    if (existingProgram) {
      throw new ConflictError('Training program with this slug already exists');
    }

    const resolvedCreatedBy = await resolveCreatedBy(createdBy);

    // Normalize programTrainingType to uppercase if provided
    const normalizedData = { ...trainingProgramData };
    if (normalizedData.programTrainingType) {
      normalizedData.programTrainingType = normalizedData.programTrainingType.toUpperCase();
    }
    ...
    logger.info(`Training program created: ${trainingProgram.title}`);
    return trainingProgram;
  } catch (error) {
    logger.error('Create training program failed', error);
    throw error;
  }
};

export const updateTrainingProgram = async (trainingProgramId: string, trainingProgramData: any) => {
  try {
    const existingProgram = await prisma.trainingProgram.findUnique({
      where: { id: trainingProgramId },
    });
    ...
    if (trainingProgramData.slug && trainingProgramData.slug !== existingProgram.slug) {
      const slugExists = await prisma.trainingProgram.findUnique({
        where: { slug: trainingProgramData.slug },
      });
      if (slugExists) {
        throw new ConflictError('Training program with this slug already exists');
    ...
    logger.info(`Training program updated: ${updatedProgram.title}`);
    return updatedProgram;
  } catch (error) {
    logger.error(`Update training program failed: ${trainingProgramId}`, error);
    throw error;
  }
};

export const deleteTrainingProgram = async (trainingProgramId: string) => {
  try {
    const existingProgram = await prisma.trainingProgram.findUnique({
      where: { id: trainingProgramId },
    });
    ...
    logger.info(`Training program deleted: ${trainingProgramId}`);
    return { message: 'Training program deleted successfully' };
  } catch (error) {
    logger.error(`Delete training program failed: ${trainingProgramId}`, error);
    throw error;
  }
};